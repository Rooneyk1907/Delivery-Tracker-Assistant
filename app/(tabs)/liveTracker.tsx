...
import MileageInput from '@/components/inputs/mileageInput';
import ClockInOut from '@/components/inputs/clockInOut';
...
export default function LiveTracker() {
	...
	const [phase, setPhase] = useState<
		'idle' | 'toRestaurant' | 'waiting' | 'toCustomer' | 'returnToHotspot'
	>('idle');
	...
	async function handleSaveToLongTerm() {
		...
		if (trackedOrder && currentWorkDay) {
			...
			const updatedTotals = calculateDayTotals(currentWorkDay.shifts, [
				...currentWorkDay.orders,
				completedOrder,
			]);
	...
	const primaryAction = (() => {
		switch (phase) {
			...
			case 'toRestaurant':
				return {
					label: 'Arrived',
					action: handleArriveAtRestaurant,
					color: styles.stop,
				};
			case 'waiting':
				return {
					label: 'Depart',
					action: handleDepartRestaurant,
					color: styles.stop,
				};
			case 'toCustomer':
				return {
					label: 'Delivered',
					action: handleOrderDelivered,
					color: styles.stop,
				};
			case 'returnToHotspot':
				return {
					label: 'Return Complete',
					action: handleReturnToHotspot,
					color: styles.start,
				};
			default:
				return { label: 'Start', action: handleStart, color: styles.start };
		}
	})();
	...
	async function handleClockOut() {
		...
		const openShiftIndex = shifts.findIndex(
			(shift) => !!shift.clockInTime && !shift.clockOutTime,
		);
				...
				{onClock ? (
					<View style={styles.card}>
								...
								{SERVICES.map((s) => (
									<TouchableOpacity
										...
										style={[
											...
											phase !== 'idle' && service === s
												? { opacity: 0.8 }
												: null,
										]}
										onPress={() => {
											if (phase === 'idle') setService(s);
										}}>
										<Text
											style={[
												styles.serviceText,
												service === s && styles.serviceTextActive,
											]}>
											{s}
										</Text>
									</TouchableOpacity>
						...
						<View style={styles.statusCard}>
							<Text style={styles.statusText}>
								{phase === 'idle'
									? 'Not tracking'
									: phase === 'toRestaurant'
										? `En route to ${restaurant || 'restaurant'}`
										: phase === 'waiting'
											? `Waiting at ${restaurant || 'restaurant'}`
											: phase === 'toCustomer'
							...
							<Text style={[styles.statusLabel, { marginTop: 6 }]}>
								Current phase
							</Text>
							<Text style={[styles.elapsed, { fontSize: 16 }]}>
								{phaseElapsedDisplay}
							</Text>
						</View>
						...
						<View style={styles.buttonRow}>
							<TouchableOpacity
								style={[styles.button, primaryAction.color]}
							...
							{phase === 'returnToHotspot' && (
								<TouchableOpacity
									...
									onPress={handleNewOrderReceived}>
