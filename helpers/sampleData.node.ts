...
function calculateTimeAfterElapsed(startTime: string, elapsedTime: number) {
  ...
  const normalizedMinutes =
    ((startMinutes + elapsedMinutes) % totalMinutesInDay + totalMinutesInDay) % totalMinutesInDay;
...
function createSampleShifts(numberofShifts: number) {
  const shifts: Array<{ clockInTime: string; clockOutTime: string; duration: number }> = [];
  ...
  for (let shift = 1; shift <= numberofShifts; shift++) {
    ...
    const clockInTime = calculateTimeAfterElapsed(prevClockOutTime, offClockDurations[shift - 1]);
...
function createSampleOrder(
  ...
  const restaurantOptions = ['McDonalds', 'Whataburger', 'Killer Burger', 'Chick-fil-A', 'Panda Express'];

  const startTime = getRandomTimeInRange(availableStartTime, availableEndTime);

  let availableDuration =
    parseDurationToSeconds(availableEndTime) - parseDurationToSeconds(startTime);

  if (availableDuration < 900) return {} as any;

  const toRestaurant = Math.floor(Math.random() * (availableDuration - 180) + 60);
  const restArrivalTime = calculateTimeAfterElapsed(startTime, toRestaurant);
  availableDuration -= toRestaurant;

  const atRestaurant = Math.floor(Math.random() * (availableDuration - 120) + 60);
  const restDepartureTime = calculateTimeAfterElapsed(restArrivalTime, atRestaurant);
  ...
  const endDeadheadTime = calculateTimeAfterElapsed(deliveryTime, returnToHotspot);

  const totalDuration = toRestaurant + atRestaurant + toCustomer + returnToHotspot;

  const service = serviceOptions[Math.floor(Math.random() * (serviceOptions.length - 1))];
  const restaurant = restaurantOptions[Math.floor(Math.random() * (restaurantOptions.length - 1))];

  const miles = Number((Math.random() * (((toRestaurant + toCustomer) / 3600) * 40) + 1).toFixed(1));
