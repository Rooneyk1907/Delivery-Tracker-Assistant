type ShiftNode = { clockInTime: string; clockOutTime: string; duration: string | number };
type OrderNode = {
  pay?: number | { gross?: number; net?: number };
  miles?: number;
...
function orderGross(order: OrderNode): number {
  if (typeof order?.pay === 'number') return order.pay;
  return Number(order?.pay?.gross ?? 0);
}

function orderNet(order: OrderNode): number {
...
export function calculateDayTotals(shifts: ShiftNode[], orders: OrderNode[]) {
  const totalShiftSeconds = shifts.reduce((sum, shift) => sum + parseDurationToSeconds(shift.duration), 0);
  const activeSeconds = orders.reduce((sum, order) => sum + parseDurationToSeconds(order.totalDuration), 0);
