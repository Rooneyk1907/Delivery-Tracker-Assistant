...
function calculateTimeAfterElapsed(startTime, elapsedSeconds) {
...
function splitOrderSegments(totalDuration) {
	...
	const baseTotal =
		minToRestaurant + minAtRestaurant + minToCustomer + minReturn;
	const extra = Math.max(0, totalDuration - baseTotal);

	const toRestaurantExtra = randInt(0, Math.floor(extra * 0.25));
	const atRestaurantExtra = randInt(0, Math.floor(extra * 0.3));
...
function pickService() {
	...
	if (roll < 0.45) return 'DoorDash';
	if (roll < 0.75) return 'UberEats';
	return 'GrubHub';
}

function pickRestaurant(service) {
	const serviceRestaurantOptions = {
...
function createSampleShifts(numberOfShifts) {
	const shifts = [];
...
function createOrderWithinShift(
	date,
	orderId,
	startMinute,
	...
	const restArrivalTime = calculateTimeAfterElapsed(
		startTime,
		segments.toRestaurant,
	);
	const restDepartureTime = calculateTimeAfterElapsed(
		restArrivalTime,
		segments.atRestaurant,
	);
	const deliveryTime = calculateTimeAfterElapsed(
		restDepartureTime,
		segments.toCustomer,
	);
	const endDeadheadTime = calculateTimeAfterElapsed(
		deliveryTime,
		segments.returnToHotspot,
	);
	...
	const roadHours =
		(segments.toRestaurant + segments.toCustomer + segments.returnToHotspot) /
		3600;
	const miles = Number(Math.max(0.8, roadHours * randFloat(16, 28)).toFixed(1));

	const totalHours = totalDurationSeconds / 3600;
	const basePayByService = {
	...
	const grossPay = round2(
		basePayByService[service] + distancePay + timePay + tip,
	);
...
function createOrdersForShift(date, shift, firstOrderId) {
	...
	for (let orderIndex = 0; orderIndex < targetOrders; orderIndex++) {
		...
		const maxThisOrderMinutes =
			minutesAfterStart - (remainingOrders - 1) * minimumOrderMinutes;
		const orderMinutes = randInt(
			minimumOrderMinutes,
			Math.min(42, maxThisOrderMinutes),
		);
		const orderDurationSeconds = orderMinutes * 60;

		const order = createOrderWithinShift(
			date,
			nextOrderId,
			orderStartMinute,
...
function createSampleDays(numberOfDays) {
	const workDayArray = [];
	...
	for (const date of workedDays) {
		...
		for (const shift of shifts) {
			const generated = createOrdersForShift(date, shift, orderId);
			orders = orders.concat(generated.orders);
			orderId = generated.nextOrderId;
		}
