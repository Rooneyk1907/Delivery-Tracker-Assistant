...
export async function seedWorkDaysFromSample(options?: {
	force?: boolean;
}): Promise<void> {
	if (!__DEV__) return;
	...
	try {
		const seededVersion = await AsyncStorage.getItem(
			DEV_SAMPLE_SEED_VERSION_KEY,
		);
		const existingRaw = await AsyncStorage.getItem(LONG_TERM_STORAGE_KEY);
		const existingDays = existingRaw
			? (JSON.parse(existingRaw) as StoredDay[])
			: [];
		...
		const seeded: StoredDay[] = sampleDays.map((day) => ({
			...day,
			savedAt: now,
		}));
...
export function workDay() {
	...
	async function getWorkDay(date: string): Promise<StoredDay | null> {
		const allWorkDays = await loadAll();
		const existingWorkDay = allWorkDays.find(
	...
	async function update(
		id: string,
		patch: Partial<StoredDay>,
	): Promise<StoredDay | undefined> {
		const allWorkDays = await loadAll();
...
export function liveTracking() {
	async function save(state: ActiveTrackingState): Promise<void> {
		try {
			await AsyncStorage.setItem(
				TEMP_STORAGE_LIVE_TRACKING_KEY,
	...
	async function update(patch: Partial<ActiveTrackingState>): Promise<void> {
		try {
			...
			const updatedActiveTrackingState: ActiveTrackingState = {
				...parsed,
				...patch,
			};
	...
	async function loadDraft(): Promise<OrderEntryDraft | null> {
		try {
			const stored = await AsyncStorage.getItem(
				TEMP_STORAGE_ORDER_ENTRY_DRAFT_KEY,
			);
			if (!stored) return null;
